...
def compute_BiC_c_comps_sets_v2(D, c_components, parent_sets, bidirect_tuple):
    '''compute the bic score'''
    # TODO fix this
    nVars = D.shape[1]
    nSamples = D.shape[0]
    ...
    district = np.where(district)[0] # list(set(c_components) - set(parent_sets)) + list(set(parent_sets))
    # scores(iComp) = logdet(2 * pi * covMat(district, district)) + (nSamples - 1) / nSamples;
    scores = compute_local_BiC_comp_sets_v2(compMag, c_components, district, len(c_components), covMat, nSamples, tol)
...
def compute_local_BiC_comp_sets_v2(compMag, component, district, compSize, CovMat, nSamples,  tol):
    ...
    if district.size == 1:
        ...
        curCovMat = CovMat[np.ix_(district, district)]
        compMag = compMag[np.ix_(district, district)]
        _, _, curHatCovMat, _ = RICF_fit_v2(compMag, curCovMat, tol)
        ...
        if np.sum(remParents) > 0:
            l1 = compSize * math.log(2 * math.pi)
            l2 = logdet_signed - math.log(np.prod(np.diag(curHatCovMat[np.ix_(parInds, parInds)])))
            l3 = (nSamples - 1) / nSamples * (np.trace( matrix_division_left_v2(curHatCovMat,curCovMat)) - np.sum(remParents))
            sc = l1 + l2 +l3
        else:
            l1 = compSize * math.log(2 * math.pi)
            l2 = logdet_signed
            l3 = (nSamples - 1) / nSamples * np.trace(matrix_division_left_v2(curHatCovMat,curCovMat))
            sc = l1 + l2 + l3
...
def compute_BiC_c_comps_sets_v3(delta_beta, D, c_components, parent_sets, bidirect_tuple):
    '''compute the bic score'''
    # TODO fix this
    nVars = D.shape[1]
    nSamples = D.shape[0]
    ...
    district = np.where(district)[0] # list(set(c_components) - set(parent_sets)) + list(set(parent_sets))
    # scores(iComp) = logdet(2 * pi * covMat(district, district)) + (nSamples - 1) / nSamples;
    scores = compute_local_BiC_comp_sets_v3(delta_beta, compMag, c_components, district, len(c_components), covMat, nSamples, tol)
...
def compute_local_BiC_comp_sets_v3(delta_beta, compMag, component, district, compSize, CovMat, nSamples,  tol):
    ...
    if district.size == 1:
        ...
        compMag = compMag[np.ix_(district, district)]
#        _, _, curHatCovMat, _ = RICF_fit(compMag, curCovMat, tol)
        cur_delta = delta_beta['delta']
        cur_beta = delta_beta['beta']
#        cur_delta = delta[np.ix_(district, district)]
#        cur_beta = beta[np.ix_(district, district)]
        I_minus_cur_delta = np.identity(cur_delta.shape[0]) - cur_delta
        I_minus_cur_delta_inv = np.linalg.inv(I_minus_cur_delta)
        relCovMat = np.matmul( np.matmul(I_minus_cur_delta_inv, cur_beta), I_minus_cur_delta_inv.T)
        curRelCovMat = relCovMat[np.ix_(district, district)]
        ...
        sign, logdet = np.linalg.slogdet(curRelCovMat)
        logdet_signed = sign* logdet
        if np.sum(remParents) > 0:
            l1 = compSize * math.log(2 * math.pi)
            l2 = logdet_signed - math.log(np.prod(np.diag(curRelCovMat[np.ix_(parInds, parInds)])))
            l3 = (nSamples - 1) / nSamples * (np.trace( matrix_division_left(curRelCovMat,curCovMat)) - np.sum(remParents))
            sc = l1 + l2 +l3
        else:
            l1 = compSize * math.log(2 * math.pi)
            l2 = logdet_signed
            l3 = (nSamples - 1) / nSamples * np.trace(matrix_division_left(curRelCovMat,curCovMat))
            sc = l1 + l2 + l3
...
def compute_local_BiC_c_component(compMag, component, district, compSize, covMat, nSamples,  tol):
    ...
    if sum(district) == 1:
        ...
        _, _, curHatCovMat, _, _ = RICF_fit(compMag, curCovMat, tol)
        remParents = district
...
def RICF_fit_v2(smm, covMat, tol):
        ...
        for iVar in range(nVars):
           ...
           if iSp.size == 0:
               if iPar.size > 0:
                   if iter==1:
                       if len(vcomp) == 1:
                           ...
                           xtemp = np.linalg.solve(covMat[np.ix_(iPar, iPar)].T,covMat[iVar, iPar].T)
                           #inv_mat =   np.linalg.inv(covMat[np.ix_(iPar, iPar)])
                           #beta[iVar, iPar] = np.matmul(-covMat[iVar, iPar],inv_mat) # TODO check this
                           beta[iVar, iPar] = - xtemp.T
                       omega[iVar, iVar] = covMat[iVar, iVar] + np.matmul(beta[iVar, iPar],covMat[iPar, iVar])
               ...
               if len(vcomp) == 1:
                   ...
                   xtemp = np.zeros((nVars, nVars))
                   betacols = list(range(0,beta.shape[1]))
                   xtemp[np.ix_(vcomp, betacols)] = np.linalg.solve(omega[np.ix_(vcomp, vcomp)],beta[vcomp, :])
                   Z = xtemp[iSp,:]
                   #oInv[np.ix_(vcomp, vcomp)] = np.linalg.inv(omega[np.ix_(vcomp, vcomp)]) #; %\Omega_{-i, -i}^-1
               #Z = np.matmul(oInv[np.ix_(iSp, vcomp)], beta[vcomp, :])
               if Z.ndim == 1:
               ...
               if YX.size == 1:
                   ...
                   temp = matrix_division_right_v2(YX.T, XX) #np.lin.solve(YX.dot(YX.T), YX).dot(XX)  # YX.T/XX # TODO check
                   # % update beta, omega
                   beta[iVar, iPar] = -temp[0,range1]
                   omega[iVar, iSp] = temp[0, range2]
                   omega[iSp, iVar] = omega[iVar, iSp]
                   ...
                   if iSp.size == 1:
                       omega[iVar, iVar] = tempVar + omega[iVar, iSp]/ omega[np.ix_(iSp, iSp)] * omega[iSp, iVar]
                        ...
                        omega[iVar, iVar] = tempVar + np.matmul(matrix_division_right_v2(omega[iVar, iSp], omega[np.ix_(iSp, iSp)])\
                                       ,omega[iSp, iVar])
...
#                       xtemp = np.linalg.solve(omega[np.ix_(vcomp, vcomp)],omega)
#                       now do np.matmul(omega[[iVar],[iSp]], xtemp)
#                       the problem with this approach is that oInv was updated above for vcomp and now we use iSp
                       omega[iVar, iVar] = tempVar + \
                           np.matmul(np.matmul(omega[[iVar], [iSp]], oInv[np.ix_(iSp,iSp)]), omega[iSp, [iVar]])
                                       # omega[iVar, iSp] * oInv[iSp, iSp] * omega[iSp, iVar]

               else:
                   omega[iVar, iSp] =  matrix_division_right_v2(YX.T, XX) # YX'/XX;
                   omega[iSp, iVar] = omega[iVar, iSp]
                   tempVar = covMat[iVar, iVar] - np.matmul(omega[iVar,iSp],YX)
                   omega[iVar, iVar] = tempVar+ \
                                       np.matmul(np.matmul(omega[iVar, iSp],oInv[np.ix_(iSp,iSp)]),omega[iSp, iVar])
    ...
    # %    end
    #hatCovMat = np.matmul( np.matmul(np.linalg.inv(beta), omega), np.linalg.inv(beta.T))
    xtemp = np.linalg.solve(beta,omega)
    ytemp = np.linalg.solve(beta,xtemp.T)
    hatCovMat = ytemp.T
    # % load('maria.mat')
    # % kk = vertcat(kk,[a b c d e]);
    # % [a b c d]
...
def matrix_division_right_v2(YX, XX):
    # solve for YX/XX or x XX = YX

    xtemp = np.linalg.solve(XX.T,YX.T)
    x = xtemp.T
...
def matrix_division_left_v2(B, b):
    # solves for B\b or Bx = b
