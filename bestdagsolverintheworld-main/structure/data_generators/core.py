...
from structure.categorical_variable_mapper import (
...
def generate_structure(
    ...
    # !!! The original dynotears generator is somehow wrong and generates too nice graphs for dynotears. - Replaced by the generator from notears.
    if num_nodes < 2:
    ...
    elif generator == 'notears':
        if graph_type == "erdos-renyi":
            gt = 'ER'
        elif graph_type == "barabasi-albert":
            gt = 'SF'
        s0 = round((degree / 2) * num_nodes)
        B_true = utils.simulate_dag(num_nodes, s0, gt)
        W_true = utils.simulate_parameter(B_true)
        graph = StructureModel(W_true)
    else:
        assert False
