...
def check_for_mag(model, where):
    ...
    if where == GRB.Callback.MIPSOL:
        ...
        #weights_vals = model.cbGetSolution(model._edges_weights)
        directed_weights_vals = model.cbGetSolution(model._directed_weights)
        bidirected_weights_vals = model.cbGetSolution(model._bidirected_weights)
        ...
        if not constr_added and model._B_ref is not None and (
            ...
            W_sol = extract_adj_matrix(edges_vals, directed_weights_vals, model._d)
            Wbi_sol = extract_adj_matrix(biedges_vals, bidirected_weights_vals, model._d)
...
def solve(X, lambda1, loss_type, reg_type, w_threshold, tabu_edges={}, B_ref=None, mode='shortest_cycle',
    ...
    for v1 in range(d):
        for v2 in range(d):
            if v1 != v2:
                ...
                directed_weights[v1, v2] = m.addVar(lb=float('-inf'), vtype=GRB.CONTINUOUS, name=f'weight{v1}->{v2}')
                bidirected_weights[v1, v2] = m.addVar(lb=float('-inf'), vtype=GRB.CONTINUOUS, name=f'weight{v1}->{v2}')

                if constraints_mode == 'no-weights':
                    #m.addConstr(edges_weights[v1, v2] == (edges_vars[v1, v2] + biedges_vars[v1, v2]))
                    m.addConstr(directed_weights[v1, v2] == edges_vars[v1, v2])
                    m.addConstr(bidirected_weights[v1, v2] == biedges_vars[v1, v2])
                else:
                    #m.addConstr(edges_weights[v1, v2] <= weights_bound * (edges_vars[v1, v2] + biedges_vars[v1, v2]))
                    #m.addConstr(-edges_weights[v1, v2] <= weights_bound * (edges_vars[v1, v2] + biedges_vars[v1, v2]))
                    m.addConstr(directed_weights[v1, v2] <= weights_bound * edges_vars[v1, v2])
                    m.addConstr(-directed_weights[v1, v2] <= weights_bound * edges_vars[v1, v2])
                    m.addConstr(bidirected_weights[v1, v2] <= weights_bound * biedges_vars[v1, v2])
                    m.addConstr(-bidirected_weights[v1, v2] <= weights_bound * biedges_vars[v1, v2])
    ...
    # regulazition
    if reg_type == 'l2':
        reg = gp.quicksum(w ** 2 for w in directed_weights.values()) + gp.quicksum(w ** 2 for w in bidirected_weights.values())
    elif reg_type == 'l1':
    ...
    # Cost function
    # X-XW_D var - predicted by the directed edges ...
    xmxwd_vars = {}
    for i in range(n):
        for j in range(d):
            xmxwd_vars[i, j] = m.addVar(vtype=GRB.CONTINUOUS, name=f'X-XW_D{i}-{j})', lb=float('-inf'))
            m.addConstr(xmxwd_vars[i, j] == X[i, j] - gp.quicksum(X[i, k] * directed_weights[k, j] for k in range(d) if k != j))
    if loss_type == 'l2':
        if robust:
            m.setObjective(robust_objective + lambda1 * reg / d, GRB.MINIMIZE)
        else:
            #m.setObjective(gp.quicksum(
            #    (xmxwd_vars[i, j] - gp.quicksum(xmxwd_vars[i,k] * bidirected_weights[k, j] for k in range(d) if k != j)) ** 2 for i in
            #    range(n) for j in range(d)) / n + lambda1 * reg / d, GRB.MINIMIZE)
            err_vars = {}
            for i in range(n):
                for j in range(d):
                    err_vars[i, j] = m.addVar(vtype=GRB.CONTINUOUS, name=f'error term{i}-{j})', lb=float('-inf'))
                    m.addConstr(err_vars[i, j] == xmxwd_vars[i, j] - gp.quicksum(xmxwd_vars[i,k] * bidirected_weights[k, j] for k in range(d) if k != j))

            m.setObjective(gp.quicksum( err_vars[i, j] ** 2
                    for i in range(n) for j in range(d)) / n + lambda1 * reg / d, GRB.MINIMIZE)
        ...
        for i in range(n):
            for j in range(d):
                ...
                m.addConstr((xmxwd_vars[i, j] - gp.quicksum(xmxwd_vars[i, k] * bidirected_weights[k, j] for k in range(d) if k != j)) <= abs_vars[i, j])
                m.addConstr(-(xmxwd_vars[i, j] - gp.quicksum(xmxwd_vars[i, k] * bidirected_weights[k, j] for k in range(d) if k != j)) <= abs_vars[i, j])
    ...
    m._directed_weights = directed_weights
    m._bidirected_weights = bidirected_weights
    m._lazy_count = 0
    ...
    W = tupledict_to_np_matrix(m.getAttr('x', directed_weights), d)
    Wbi = tupledict_to_np_matrix(m.getAttr('x', bidirected_weights), d)
